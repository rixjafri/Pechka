...
class FollowingVC: BaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return APPDELEGATE.getFollowingList.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
      ...
      let cell = tbleView.dequeueReusableCell(withIdentifier: "FollowingTbleViewCell") as! FollowingTbleViewCell
        if let tempDic1 : Dictionary = APPDELEGATE.getFollowingList[indexPath.row] as? Dictionary<String,Any>
         {
            if let getCatName : String = tempDic1["added_date"] as? String
            {
                cell.lblJoined.text = "\(NSLocalizedString("Joined", comment: "")) - \(getCatName)"
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if let tempDic1 : Dictionary = APPDELEGATE.getFollowingList[indexPath.row] as? Dictionary<String,Any>
         {
            if let getCatName : String = tempDic1["user_id"] as? String
            {
                objRef.followUserId = getCatName
            }
            if let getCatName : String = tempDic1["user_name"] as? String
            {
                objRef.userName = getCatName
            }
        }
      ...
      Alamofire.request(BaseViewController.API_URL+"rest/userfollows/search/api_key/teampsisthebest/offset/0/limit/6", method: .post , parameters: parametersList,headers:headers1).responseJSON{ response in
        self.objHudHide();
            ...
            case .success (let value):
                ...
                APPDELEGATE.getFollowingList.removeAllObjects()
                ...
                if let array = response.result.value as? [Any] {
                    ...
                    {
                        APPDELEGATE.getFollowingList .insert(resultsArray[i], at:  APPDELEGATE.getFollowingList .count)
               ...
               print("Recent Items:\(APPDELEGATE.getFollowingList)")
