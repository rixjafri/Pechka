...
class FavouritesVC: BaseViewController,UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
...
@IBAction func action_itemTapBtnTapped(_ sender: UIButton)
{
...
func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
     ...
     return APPDELEGATE.getFavtems.count
}
...
func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
{
 ...
 let cell:RecentPopulerItemsCollectionViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: "RecentPopulerItemsCollectionViewCell", for: indexPath) as! RecentPopulerItemsCollectionViewCell
    ...
    if let tempDic1 : Dictionary = APPDELEGATE.getFavtems[indexPath.row] as? Dictionary<String,Any>
     {
        ...
        if let tempDicUser : Dictionary = tempDic1["user"] as? Dictionary<String,Any>
         {
//                    if let getSliderNameImg : String = tempDicUser["user_profile_photo"] as? String
//                    {
//                        var imageUrl =  BaseViewController.IMG_URL
...
//                        cell.imgUser.layer.cornerRadius = cell.frame.height/2
...
func getFavouritesApi()
 {
  ...
  let parametersList:[String : Any] = [
    ...
    objHudShow()
    let userId = defaultValues.value(forKey: "UserID") ?? ""
  Alamofire.request(BaseViewController.API_URL+"rest/items/get_favourite/api_key/teampsisthebest/login_user_id/\(userId)/limit/30/offset/0", method: .get , parameters: parametersList,headers:headers1).responseJSON{ response in
    self.objHudHide();
